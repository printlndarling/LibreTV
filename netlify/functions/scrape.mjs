// /netlify/functions/scrape.mjs - Netlify Function (zero external dependencies)
// 纯正则实现，无需 cheerio
...
export default async function handler(event, context) {
  ...
  try {
    ...
    if (!source && event.headers['x-netlify-original-pathname']) {
      const p = event.headers['x-netlify-original-pathname'].split('/').filter(Boolean);
      if (p.length >= 4) { source = p[2]; action = p[3]; }
    }
    if (!source && event.rawUrl) {
      try { const u = new URL(event.rawUrl); const p = u.pathname.split('/').filter(Boolean); if (p.length >= 4) { source = p[2]; action = p[3]; } } catch(e) {}
    }
    let result;
    ...
    else return { statusCode: 400, headers, body: JSON.stringify({ error: 'Unknown source' }) };
